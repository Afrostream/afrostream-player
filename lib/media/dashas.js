...
videojs.Dashas = videojs.Flash.extend({
  ...
  init: function (player, options, ready) {
    ...
    var customData = drmData['httpRequestHeaders'].customData;
    var serverUrl = drmData['serverURL'];
    ...
    options.flashVars = videojs.util.mergeOptions({
      ...
      'language': 'fr',
      'metadataUrl': encodeURIComponent(videojs.Dashas.buildMetadataUrl(options.protData)),
      'authenticationToken': encodeURIComponent(videojs.Dashas.buildOptData(options.protData)),
      'spinner': !1,
      'watermark': !1,
      'debug': true,
      'quality': 'autolow',
...
videojs.Dashas.prototype.src = function (src) {
  ...
  this.player().ready(function () {
    ...
    //tech.el().vjs_src(src);
    this.el_.vjs_source(src, Z, K, R);
  });
};

videojs.Dashas.prototype.setSrc = function (src) {
  // Make sure source URL is absolute.
  src = videojs.getAbsoluteURL(src);
  var autoPlay = this.player_.autoplay();
  var serverUrl = videojs.Dashas.buildMetadataUrl(this.options().protData);
  var customData = videojs.Dashas.buildOptData(this.options().protData);
...
videojs.Dashas.getDRMData = function (protData) {
  var drmData = protData['com.adobe.flashaccess'];
...
videojs.Dashas.buildMetadataUrl = function (protData) {
  var data = videojs.Dashas.getDRMData(protData);
...
videojs.Dashas.buildOptData = function (protData) {
  return videojs.Dashas.getDRMData(protData).customData
};
